...
class ResidualBlockLast(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        super(ResidualBlockLast, self).__init__()
...
class ResNetCAM(nn.Module):
    def __init__(self, hidden_sizes, num_blocks, input_dim=1000,
        ...
        for idx, hidden_size in enumerate(hidden_sizes):
            if(idx < len(hidden_sizes)-2):
                ...
                layers.append(self._make_layer(hidden_size, num_blocks[idx],
                stride=strides[idx], last=True))
                self.sizeKernel = num_blocks[idx]
    ...
    def encode(self, x):
        ...
        x = self.encoder(x)
        x = F.avg_pool1d(x, self.sizeKernel)
        z = x.view(x.size(0), -1)
        return z
    ...
    def _make_layer(self, out_channels, num_blocks, stride=1, last=False):
        if not last:
            ...
            blocks = []
            for stride in strides:
                blocks.append(ResidualBlockLast(self.in_channels, out_channels,
                    stride=stride))
                self.in_channels = out_channels 
